...
foreign import javascript unsafe "($3).set($1, $2)"
  fms_setCurve :: Double -> FamoObj Curve -> Opacity -> IO ()
foreign import javascript unsafe "($4).set($1, $2, $3)"
  fms_setCurveCb :: Double -> FamoObj Curve -> Callback (IO ()) -> Opacity -> IO ()
...
set ov c (Just f) op = do jsc <- toJSRef c
                          cb <- syncCallback ContinueAsync f
